...
DCI.SpatialQuery = {
    ...
    */
    Html: "<!-- 框选菜单部分 -->" +
           ...
           "<div class='spatialquery-layer'>" +
               "<fieldset style='width:200px;padding:5px;border:1px solid #1BB0F5;margin-left:1px;margin-top:5px;'>" +
    ...
    */
    Init: function (map) {
        ...
        //空间查询列表初始化
        DCI.SpatialQuery.enableAnalysisLayers = [{ aliasName: "调压器", allFeatureFields: "TADDRESS", featureLayerName: MapConfig.searchMapUrl + "/0", type: "0" },
                                                 { aliasName: "所有图层", allFeatureFields: "FID", featureLayerName: MapConfig.searchMapUrl, type: "1" },
                                                 { aliasName: "可见图层", allFeatureFields: "FID", featureLayerName: MapConfig.searchMapUrl, type: "2" }];
    ...
    */
    ...
    /**
     * 选择勾选的图层
     * 设置了只能勾选一项，要么是指定图层调压器，要么查询所有的图层
    */
    ...
    */
    doSpatialSearch: function (geometry) {
        ...
        for (var i = 0; i < DCI.SpatialQuery.layerNames.length; i++) {
            ...
            } else if (DCI.SpatialQuery.type[i] == "1" || DCI.SpatialQuery.type[i] == "2") {//所有图层的空间查询
            ...
            } else if (DCI.thematicmap.sign == 1){
                DCI.SpatialQuery.spatialQuery.returnFields = "FID";//获取Query空间查询的outfield
                DCI.SpatialQuery.spatialQuery.layerName = DCI.SpatialQuery.layerNames[i];//获取Query空间查询的服务URL
                DCI.SpatialQuery.searchSP(geometry);
            }
        }
    ...
    */
    searchIdentify: function (geometry) {
        ...
        identifyParams.layerIds = [0, 1, 2, 3, 4, 5,6];//设置查询图层列表
        if(DCI.SpatialQuery.type[0] == "1"){
        ...
        }else if (DCI.SpatialQuery.type[0] == "2"){
    ...
    identifyInfo: function (results) {
        ...
        if (results.length > 0) {
            for (var i = 0; i < DCI.SpatialQuery.pageSize; i++) {
                ...
                innerStr.push('<p>');
                innerStr.push('<a onclick="DCI.SpatialQuery.toLocation(' + i + ',\'' + tempID + '\',\'' + results[infactItem].feature.attributes.TADDRESS + '\')">' + results[infactItem].feature.attributes.TADDRESS + '</a><br/>');
                innerStr.push('</p>');
                innerStr.push('</div>');
                innerStr.push('</div>')
                innerStr.push('</div>');
    ...
    navInfo: function (results) {
        ...
        if (results.features.length > 0) {
            for (var i = 0; i < DCI.SpatialQuery.pageSize; i++) {
                ...
                innerStr.push('<div class="left_list_li1">');
                innerStr.push('<p>');
                innerStr.push('<a onclick="DCI.SpatialQuery.toLocation(' + i + ',\'' + tempID + '\',\'' + results.features[infactItem].attributes.TADDRESS + '\')">' + results.features[infactItem].attributes.TADDRESS + '</a><br/>');
                innerStr.push('</p>');
                innerStr.push('</div>');
                innerStr.push('</div>')
                innerStr.push('</div>');
