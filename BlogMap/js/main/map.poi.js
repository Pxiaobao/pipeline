...
DCI.Poi = {
    ...
    InitHtml:"<div id='list_container' style='margin-top:5px;display:block'>" +
                                                   ...
                                                   "<div class='class-lists-item odd'><div class='img-list-item food'/><span class='text' name='调压器'>调压器</span></div>"+
                                                   "<div class='class-lists-item even'><div class='img-list-item hotel'/><span class='text' name='中压节点'>中压节点</span></div>"+
                                                   "<div class='class-lists-item odd'><div class='img-list-item bank'/><span class='text' name='门站'>门站</span></div>"+
                                                   "<div class='class-lists-item even'><div class='img-list-item service'/><span class='text' name='中压管线'>中压管线</span></div>"+
                                                   "<div class='class-lists-item odd'><div class='img-list-item education'/><span class='text' name='高压管线'>高压管线</span></div>"+
                                                   "<!--div class='class-lists-item even'><div class='img-list-item hospital'/><span class='text' name='医疗服务'>医疗服务</span></div-->" +
                                                 "</div>" +
                                                   "</div></div>" + "<!-- 兴趣点搜索结束符 -->"+
    ...
    Init: function (map) {
        ...
        function InitEvent() {//初始化绑定界面控件事件
            ...
            $(".class-lists-item span").bind("click", function () {
                ...
                DCI.Poi.keywordType = $(this).attr("name");
                DCI.Poi.isTpyeSearch = true;
                DCI.Poi.search(DCI.Poi.keywordType, DCI.Poi.pageIndex, DCI.Poi.pageSize);//分类查询
            });
    ...
    autoComple: function (elementID) {
        ...
        $("#" + elementID).autocomplete({
            source: function (request, response) {
                var data = [];//初始化结果数组
                var find = new esri.tasks.FindTask(MapConfig.searchMapUrl + "/");
                var params = new esri.tasks.FindParameters();
                params.layerIds = [0, 1, 2, 3, 4, 5, 6];
                params.searchFields = ["taddress"];
                ...
                function navInfoatuo(result) {
                    if (result.length > 0) {
                        for (var i = 0; i < result.length; i++) {
                            if (i == 10) //不够十条
                                break;
                            data.push(result[i].feature.attributes.taddress);
    ...
    search: function (keyword, pageIndex, pageSize) {
        ...
        $("#listpages").css({ display: "none" });
            ...
            if (!DCI.Poi.isTpyeSearch) {//FindTask查询
                var find = new esri.tasks.FindTask(MapConfig.searchMapUrl + "/");//URL
                var params = new esri.tasks.FindParameters();
                params.layerIds = [0,1,2,3,4,5,6]; //设置查询图层列表
                params.searchFields = ["TADDRESS"]; //设置查询图层的字段,根据NAME字段来模糊查询
                ...
                query.outFields = ["TADDRESS"];
                query.where = "1=1";
                switch (keyword)//判断查询服务
                {
                    case "调压器":
                        typeUrl = MapConfig.searchMapUrl + "/" + 4;
                        break;
                    case "中压节点":
                        typeUrl = MapConfig.searchMapUrl + "/" + 1;
                        break;
                    case "门站":
                        typeUrl = MapConfig.searchMapUrl + "/" + 2;
                        break;
                    case "中压管线":
                        typeUrl = MapConfig.searchMapUrl + "/" + 5;
                        break;
                    case "高压管线":
                        typeUrl = MapConfig.searchMapUrl + "/" + 6;
                        break;
    ...
    findInfo: function (results) {
        ...
        if (results.length > 0) {
            ...
            for (var i = 0; i < DCI.Poi.pageSize; i++) {
                ...
                var attr = { "title": results[infactItem].feature.attributes.taddress, "content": results[infactItem].feature.attributes.taddress };
                ...
                innerStr.push('<p>');
                innerStr.push('<a onclick="DCI.Poi.toLocationPOI(' + i + ',\'' + tempID + '\',\'' + results[infactItem].feature.attributes.taddress + '\')">' + results[infactItem].feature.attributes.taddress + '</a><br/>');
                innerStr.push('</p>');
                innerStr.push('</div>');
                innerStr.push('</div>')
                innerStr.push('</div>');
    ...
    navInfo: function (results) {
        ...
        if (results.features.length > 0) {
            ...
            for (var i = 0; i < DCI.Poi.pageSize; i++) {
                ...
                var attr = { "title": results.features[i].attributes.TADDRESS, "content": results.features[i].attributes.TADDRESS };
                ...
                innerStr.push('<div class="left_list_li1">');
                innerStr.push('<p>');
                innerStr.push('<a onclick="DCI.Poi.toLocationPOI(' + i + ',\'' + tempID + '\',\'' + results.features[infactItem].attributes.TADDRESS + '\')">' + results.features[infactItem].attributes.TADDRESS + '</a><br/>');
                innerStr.push('</p>');
                innerStr.push('</div>');
                innerStr.push('</div>')
                innerStr.push('</div>');
            ...
            $("#listpages").PageOperator({
                ...
                callback: function (pageIndex) {
                    //var keyword = $("#skeyword").val();
                    DCI.Poi.pageIndex = pageIndex;
                    DCI.Poi.search(DCI.Poi.keywordType, pageIndex, DCI.Poi.pageSize);
                }
            });
        } else {
            alert("搜索不到相关数据");
