...
DCI.interPolate = {
    ...
    Init: function (map) {
        ...
        $("#interpolateLayer input").bind("click", function () {
            if (this.checked) {
                ...
                var hour_start = hour.value.replace(/:/, "");
                var hour_end = hour2.value.replace(/:/, "");
                freq = DCI.interPolate.times.indexOf(hour_end)
                y = date.substr(0, 4);
                m = date.substr(5, 2);
                d = date.substr(8, 2);

                k = DCI.interPolate.times.indexOf(hour_start);
                DCI.interPolate.showLegend();
                DCI.interPolate.loopss();
                this.disabled = "disabled";
    ...
    loops: function () {
        ...
        if (i <= freq) {
            ...
            -31530.9161542535, 2722.90523641796, 1489.14988587857, 27487.954766517, 2379, 1, i);
            i++;
    ...
    loopss: function () {
        ...
        if (k <= freq) {
            DCI.interPolate.addImageLayer('layer', MapConfig.testpngUrl + y + '/' + m + '/' + d + '/' + DCI.interPolate.times[k] + '/KPA/KPA.png',
            -39233.354865437, 1174.78395606861, 632.262963602581, 31073.9973278483, 2379, 0.9, k);
            k++;
